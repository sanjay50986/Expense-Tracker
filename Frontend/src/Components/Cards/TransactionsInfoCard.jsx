...
const TransactionsInfoCard = ({ key,
    title,
    Icons,
    date,
    amount,
    ...
    return (
        <div className='group relative flex items-center gap-4 mt-2 p-3 rounded-lg hover:bg-gray-100/50'>
            <div className='w-12 h-12 flex items-center justify-center text-xl text-gray-800 bg-gray-100  rounded-full'>
                {Icons ? (
                    <img src={Icons} alt={title} className='w-6 h-6' />
                ) : (
                    <LuUtensils />
                )}
            </div>
            ...
            <div className=' flex-1 flex items-center justify-between'>
                ...
                <div className='flex items-center gap-2'>
                    {!hideDeleteBtn && (
                        <button
                        onClick={onDelete} className='text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer'>
                            <LuTrash2 size={18} />
                        </button>
                    )}
